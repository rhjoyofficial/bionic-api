    ...
    <div x-show="!loading && order">
        ...
        <div class="flex items-start justify-between mb-6">
            ...
            <div class="flex items-center gap-2">
                <button x-show="order?.is_editable && !editMode"
                        @click="enterEditMode()"
                ...
                <button x-show="editMode"
                        @click="cancelEdit()"
                        class="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition">
                ...
                <div x-show="editMode" x-cloak class="space-y-4">
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl p-4">
                        ...
                        <div x-show="searchResults.length > 0" x-cloak
                            ...
                            <template x-for="result in searchResults" :key="(result.variant_id || '') + '-' + (result.combo_id || '')">
                                <button @click="addItemFromSearch(result)"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-800 truncate" x-text="result.product_name"></p>
                                        <p class="text-xs text-gray-500" x-text="result.variant_title + (result.sku ? ' — ' + result.sku : '')"></p>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <p class="text-sm font-semibold text-gray-800">৳<span x-text="Number(result.price).toLocaleString()"></span></p>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Product</th>
                                        <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase w-32">Qty</th>
                                        <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Unit Price</th>
                                        <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase w-16">Remove</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50">
                                    <template x-for="(item, idx) in editItems" :key="idx">
                                        <tr :class="item._removed ? 'bg-red-50 opacity-50' : ''">
                                            <td class="px-5 py-3">
                                                <p class="font-medium text-gray-800" x-text="item.product_name_snapshot || item.product_name"></p>
                                                <p class="text-xs text-gray-400" x-text="item.variant_title_snapshot || item.variant_title"></p>
                                                <p x-show="item.sku_snapshot || item.sku" class="text-xs text-gray-400 font-mono" x-text="item.sku_snapshot || item.sku"></p>
                                            </td>
                                            <td class="px-5 py-3 text-center">
                                                <div class="flex items-center justify-center gap-1">
                                                    ...
                                                    <button @click="changeQty(idx, 1)" :disabled="item.quantity >= (item.max_quantity || 999)"
                                                            class="w-7 h-7 flex items-center justify-center rounded bg-gray-100 hover:bg-gray-200 disabled:opacity-30 text-xs transition">
                                                        <i class="fas fa-plus"></i>
                                                    </button>
                                                </div>
                                                <p class="text-xs mt-1" :class="item.quantity > (item.max_quantity || 999) ? 'text-red-500' : 'text-gray-400'"
                                                   x-text="'Max: ' + (item.max_quantity || '—')"></p>
                                            </td>
                                            <td class="px-5 py-3 text-right text-gray-700">
                                                ৳<span x-text="Number(item.unit_price || item.price || 0).toLocaleString()"></span>
                                            </td>
                                            <td class="px-5 py-3 text-center">
                    ...
                    <template x-if="previewData">
                        <div class="bg-white border border-green-200 rounded-xl overflow-hidden">
                            <div class="px-5 py-3 bg-green-50 border-b border-green-100">
                                <h4 class="text-sm font-bold text-green-800"><i class="fas fa-calculator mr-1"></i> Recalculated Totals</h4>
                            </div>
                            <div class="p-5 space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Subtotal</span>
                                    <span class="font-medium">৳<span x-text="Number(previewData.subtotal).toLocaleString()"></span></span>
                                </div>
                                <div x-show="previewData.tier_discount > 0" class="flex justify-between text-green-700">
                                    <span>Tier Discount</span>
                                    <span>−৳<span x-text="Number(previewData.tier_discount).toLocaleString()"></span></span>
                                </div>
                                <div x-show="previewData.coupon_discount > 0" class="flex justify-between text-green-700">
                                    <span>Coupon (<span x-text="previewData.coupon_code"></span>)</span>
                                    <span>−৳<span x-text="Number(previewData.coupon_discount).toLocaleString()"></span></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Shipping</span>
                                    <span class="font-medium">৳<span x-text="Number(previewData.shipping_cost).toLocaleString()"></span></span>
                                </div>
                                <div class="flex justify-between border-t border-gray-200 pt-2">
                                    <span class="font-bold text-gray-800">Grand Total</span>
                                    <span class="font-bold text-lg text-gray-900">৳<span x-text="Number(previewData.grand_total).toLocaleString()"></span></span>
                                </div>
                            </div>
                            ...
                            <template x-if="previewData.changes && previewData.changes.length > 0">
                                <div class="px-5 pb-4">
                                    ...
                                    <div class="space-y-1">
                                        <template x-for="change in previewData.changes" :key="change.name + change.type">
                    ...
                    {{-- Edit Error --}}
                    <div x-show="editError" x-cloak class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
                ...
                <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <span class="text-xs px-2 py-0.5 rounded-full"
                              :class="(order?.shipments?.length ?? 0) > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'"
                    ...
                    <div class="p-5 border-b border-gray-100">
                        <div class="flex items-end gap-3">
                            ...
                            <button @click="assignCourier()" :disabled="!selectedCourier || assigningCourier"
                                ...
                                <i class="fas" :class="assigningCourier ? 'fa-spinner fa-spin' : 'fa-paper-plane'"></i>
                                <span x-text="assigningCourier ? 'Creating…' : 'Create Shipment'"></span>
                            </button>
                        </div>
                        <div x-show="courierError" x-cloak class="mt-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2" x-text="courierError"></div>
                    </div>
                        ...
                        <template x-for="shipment in order?.shipments ?? []" :key="shipment.id">
                            <div class="p-5">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-semibold text-gray-800" x-text="shipment.courier_label"></span>
                                        <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                                              :class="shipmentStatusColor(shipment.status)"
                                              x-text="shipment.status_label"></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button @click="syncShipmentStatus(shipment.id)" :disabled="shipment.is_terminal"
                                ...
                                <div class="grid grid-cols-2 gap-3 text-xs">
                                    <div>
                                        ...
                                        <p class="font-mono font-medium text-gray-800" x-text="shipment.tracking_code || '—'"></p>
                                    </div>
                                    <div>
                                        <span class="text-gray-500">Consignment ID</span>
                                        <p class="font-mono font-medium text-gray-800" x-text="shipment.consignment_id || '—'"></p>
                                    </div>
                                    <div x-show="shipment.delivery_fee !== null">
                                        <span class="text-gray-500">Delivery Fee</span>
                                        <p class="font-medium text-gray-800">৳<span x-text="Number(shipment.delivery_fee || 0).toLocaleString()"></span></p>
                                    </div>
                                    <div x-show="shipment.cod_amount > 0">
                                        <span class="text-gray-500">COD Amount</span>
                                        <p class="font-medium text-gray-800">৳<span x-text="Number(shipment.cod_amount || 0).toLocaleString()"></span></p>
                                    </div>
                                </div>

                                <div class="mt-3 flex items-center justify-between text-xs text-gray-400">
                                    <span x-text="shipment.courier_status_message || ''"></span>
                                    <span x-show="shipment.status_synced_at" x-text="'Synced: ' + (shipment.status_synced_at ? new Date(shipment.status_synced_at).toLocaleString('en-GB', {day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '')"></span>
                                </div>
                                <div class="mt-1 text-xs text-gray-400" x-show="shipment.created_by_name">
                                    Created by <span class="font-medium text-gray-600" x-text="shipment.created_by_name"></span>
                                    on <span x-text="new Date(shipment.created_at).toLocaleString('en-GB', {day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})"></span>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="space-y-2 text-sm" x-data="{ s: order.shipments[0] }">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                      :class="shipmentStatusColor(s.status)"
                                      x-text="s.status_label"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Tracking</span>
                                <span class="font-mono text-xs font-medium text-gray-700" x-text="s.tracking_code || '—'"></span>
                            </div>
                        </div>
                    </div>
                </template>
