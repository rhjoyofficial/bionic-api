    ...
    <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex gap-6">
            ...
            <button @click="switchTab('{{ $t }}')"
                    :class="tab === '{{ $t }}' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'"
            ...
            <template x-for="role in roles" :key="role.id">
                <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition">
                    ...
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <div class="flex items-center gap-1" x-show="!role.is_protected">
                            ...
                            <button @click="deleteModal = { open: true, role: role }" title="Delete"
                                    ...
                                    :class="role.users_count > 0 ? 'opacity-30 cursor-not-allowed' : 'hover:text-red-600 hover:bg-red-50'"
                    ...
                    <div class="flex flex-wrap gap-1 mb-3 min-h-[24px]">
                        ...
                        <span x-show="(role.permissions ?? []).length === 0"
                              class="text-xs text-gray-400 italic">No permissions</span>
                    </div>

                    {{-- Footer actions --}}
                    <div class="flex gap-2 pt-3 border-t border-gray-100">
                        <button @click="openUsersModal(role)"
                                class="flex-1 text-xs text-center py-1.5 text-indigo-600 hover:bg-indigo-50 rounded transition"
                                x-show="role.users_count > 0">
                            <i class="fas fa-users mr-1"></i>
                            View <span x-text="role.users_count"></span> Users
                        </button>
                        <button @click="switchTab('matrix')"
                                class="flex-1 text-xs text-center py-1.5 text-gray-500 hover:bg-gray-50 rounded transition">
    ...
    <div x-show="tab === 'matrix'">
        ...
        <div x-show="!matrixLoading">
            ...
            <div class="flex items-center gap-4 mb-4 text-xs text-gray-500">
                ...
                <span class="flex items-center gap-1.5">
                    <span class="w-4 h-4 rounded bg-indigo-100 inline-block border border-indigo-200"></span> System (auto-granted)
                </span>
                ...
                <table class="text-xs border-collapse">
                    <thead>
                        ...
                        <tr class="border-b border-gray-200">
                            <th class="sticky left-0 z-20 bg-white px-4 py-3 text-left text-xs font-semibold text-gray-700 min-w-[140px] border-r border-gray-200">
                                Role
                            </th>
                            <template x-for="[group, perms] in Object.entries(matrixGroups)" :key="group">
                                <th :colspan="perms.length"
                                    ...
                                    :class="groupColors[group] ?? 'text-gray-600'"
                                    x-text="group"></th>
                            </template>
                        </tr>
                        ...
                        <tr class="border-b-2 border-gray-200">
                            ...
                            <template x-for="[group, perms] in Object.entries(matrixGroups)" :key="'h-'+group">
                                <template x-for="perm in perms" :key="perm">
                                    <th class="px-1 py-2 text-center font-medium text-gray-500 border-r border-gray-100 whitespace-nowrap min-w-[52px]"
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50/50 transition"
                                ...
                                <td class="sticky left-0 z-10 bg-inherit px-4 py-2.5 border-r border-gray-200">
                                    <div class="flex items-center justify-between gap-2">
                                        <div>
                                            <p class="font-semibold text-gray-800 whitespace-nowrap" x-text="role.name"></p>
                                            <p class="text-gray-400 mt-0.5" x-text="role.users_count + ' user' + (role.users_count !== 1 ? 's' : '')"></p>
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <div x-show="!role.is_protected" class="flex gap-1">
                                                <button @click="grantAll(role.id)"
                                                        class="px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200 transition"
                                                        title="Grant all">All</button>
                                                <button @click="revokeAll(role.id)"
                                                        class="px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-xs hover:bg-red-200 transition"
                                ...
                                {{-- Permission cells --}}
                                <template x-for="[group, perms] in Object.entries(matrixGroups)" :key="'r'+role.id+group">
                                    <template x-for="perm in perms" :key="'c'+role.id+perm">
                                        <td class="px-1 py-2 text-center border-r border-gray-100">
                                            <button
                                                @click="role.is_protected ? null : toggleCell(role.id, perm)"
                                                :disabled="role.is_protected"
                                                :title="role.is_protected ? role.name + ' always has ' + perm : (matrix[role.id]?.[perm] ? 'Revoke' : 'Grant') + ': ' + perm"
                                                :class="{
                                                    'bg-indigo-600 text-white hover:bg-indigo-700': matrix[role.id]?.[perm] && !role.is_protected,
                                                    'bg-indigo-100 text-indigo-400 cursor-default': matrix[role.id]?.[perm] && role.is_protected,
                                                    'bg-gray-200 text-gray-400 hover:bg-gray-300': !matrix[role.id]?.[perm] && !role.is_protected,
                                                    'bg-gray-100 text-gray-300 cursor-default': !matrix[role.id]?.[perm] && role.is_protected,
                                                }"
                                                class="w-7 h-7 rounded flex items-center justify-center mx-auto transition">
    ...
    <div x-show="tab === 'staff'">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
            <div class="relative max-w-md">
                ...
                <input x-model="staffSearch" @input.debounce.400ms="staffPage=1; loadStaff()"
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    <select :id="'role-'+user.id"
                                        ...
                                        <template x-for="r in allRoles" :key="r">
                                            <option :value="r" :selected="r === user.primary_role" x-text="r"></option>
                                        </template>
                                    </select>
                                    <button @click="assignRole(user)"
                                            :disabled="savingRole[user.id]"
                                            class="px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition">
                                        <i class="fas fa-check" :class="savingRole[user.id] ? 'animate-pulse' : ''"></i>
                                    </button>
                                </div>
                            </td>
                        </tr>
        ...
        <div @click.outside="createModal.open = false"
            ...
            <div class="flex justify-end gap-2 mt-5">
                ...
                <button @click="createRole()" :disabled="createModal.loading || !createModal.name.trim()"
                    ...
                    <i class="fas fa-plus mr-1" :class="createModal.loading ? 'animate-spin fa-circle-notch' : ''"></i>
    ...
    <div x-show="usersModal.open" x-transition.opacity
        ...
        <div @click.outside="usersModal.open = false"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button @click="usersModal.open = false" class="text-gray-400 hover:text-gray-600">
...
function accessControl() {
    return {
        ...
        saveTimers:    {},
        groupColors: {
            category: 'text-blue-600',   product: 'text-green-600',
            order:     'text-purple-600', coupon: 'text-pink-600',
            shipping:  'text-orange-600', customer: 'text-teal-600',
            notification: 'text-indigo-600', system: 'text-gray-600',
            analytics: 'text-cyan-600',   role: 'text-red-600',
        },
        ...
        permLabel(perm) {
            ...
            const labels = {
                ...
                export: 'Export', send: 'Send', manage: 'Manage', settings: 'Settings',
                webhooks: 'Webhooks', activity_log: 'Activity', deactivate: 'Deactivate',
            };
