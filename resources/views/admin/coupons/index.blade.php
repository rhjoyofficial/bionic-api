                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-5 py-3">
                                    <div class="flex items-center gap-2">
                                        <span class="font-mono text-xs font-semibold text-gray-800 tracking-wide" x-text="c.code"></span>
                                        <button @click="copyCode(c.code)"
                                            class="text-gray-400 hover:text-green-700 transition cursor-pointer text-xs" title="Copy">
                                ...
                                <td class="px-5 py-3 text-xs">
                                    ...
                                    <span class="ml-1 text-gray-400" x-text="c.type === 'percentage' ? 'off' : 'flat'"></span>
                                </td>
                                <td class="px-5 py-3 text-xs text-gray-600"
                                    x-text="c.min_purchase ? '৳' + Number(c.min_purchase).toLocaleString() : '—'">
                                </td>
                                <td class="px-5 py-3 text-xs">
                                    <span class="font-semibold text-gray-800" x-text="c.usages_count ?? c.used_count"></span>
                                    <span class="text-gray-400" x-text="c.usage_limit ? ' / ' + c.usage_limit : ' / ∞'"></span>
                                </td>
        ...
        <div @click.outside="showFormModal = false"
            ...
            <form @submit.prevent="saveCoupon()" class="px-6 py-5 space-y-4">
                ...
                <div x-show="!editingId">
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Code <span class="text-red-500">*</span></label>
                    <input type="text" x-model="form.code" :class="errors.code ? 'border-red-400' : 'border-gray-200'"
                ...
                <div x-show="editingId">
                    ...
                    <p class="font-mono font-bold text-gray-800 text-sm tracking-wide px-3 py-2 bg-gray-50 rounded-lg" x-text="form.code"></p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">
                            ...
                            <span class="text-gray-400 font-normal" x-text="form.type === 'percentage' ? '(%)' : '(৳)'"></span>
                        </label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" x-model="form.min_purchase" min="0" step="0.01" placeholder="Optional"
    ...
    <div x-show="showBulkModal" x-cloak
        ...
        <div @click.outside="showBulkModal = false"
                    ...
                    <div class="max-h-40 overflow-y-auto rounded-lg bg-white border border-green-200 p-3">
                        <div class="grid grid-cols-2 gap-1">
                            <template x-for="code in generatedCodes" :key="code">
                                <span class="font-mono text-xs text-gray-700 bg-gray-50 px-2 py-0.5 rounded" x-text="code"></span>
                            </template>
                        </div>
                    </div>
            ...
            <form x-show="generatedCodes.length === 0" @submit.prevent="doBulkGenerate()" class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Prefix <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Count <span class="text-red-500">*</span> <span class="text-gray-400 font-normal">(max 500)</span></label>
                        <input type="number" x-model="bulkForm.count" min="1" max="500" placeholder="10"
                            :class="bulkErrors.count ? 'border-red-400' : 'border-gray-200'"
                            class="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-green-600">
                        <p x-show="bulkErrors.count" class="text-xs text-red-500 mt-1" x-text="bulkErrors.count?.[0]"></p>
                    </div>
                </div>
                <p class="text-xs text-gray-400 -mt-2">Codes will look like: <span class="font-mono font-semibold text-gray-600" x-text="(bulkForm.prefix || 'SALE').toUpperCase() + 'XXXXXXXX'"></span></p>

                {{-- Type + Value --}}
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Type <span class="text-red-500">*</span></label>
                        <select x-model="bulkForm.type" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-green-600 cursor-pointer">
                            <option value="fixed">Fixed (৳)</option>
                            <option value="percentage">Percentage (%)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">
                            Value <span class="text-red-500">*</span>
                            <span class="text-gray-400 font-normal" x-text="bulkForm.type === 'percentage' ? '(%)' : '(৳)'"></span>
                        </label>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-700" x-text="bulkForm.is_active ? 'Active immediately' : 'Inactive'"></span>
                </div>
...
function couponManager() {
    const emptyForm = () => ({
        code: '', type: 'fixed', value: '', min_purchase: '',
        usage_limit: '', limit_per_user: '', start_date: '', end_date: '', is_active: true,
    });

    const emptyBulk = () => ({
        prefix: '', count: 10, type: 'fixed', value: '', min_purchase: '',
        usage_limit: '', limit_per_user: '', start_date: '', end_date: '', is_active: true,
    });
    ...
    return {
        ...
        async doBulkGenerate() {
            ...
            try {
                ...
                if (r.status === 422) { this.bulkErrors = data.errors ?? {}; return; }
                if (!r.ok) { alert(data.message ?? 'An error occurred'); return; }
